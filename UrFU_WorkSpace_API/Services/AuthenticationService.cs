...
public class AuthenticationService
{
    ...
    public AuthenticationService(IUserService userService, IConfiguration configuration, IMemoryCache memoryCache)
    {
    ...
    public Result<string> GetAccessToken(TokenRequest model)
    {
        ...
        switch (model.GrantType.ToLower())
        {
            ...
            case "password":
                ...
                break;
            default:
                return Result.Fail<string>(ErrorHandler.RenderError(ErrorType.InvalidGrantType));
        }
    ...
    {
        var isUserExist = _userService.GetUsersByCondition(u => u.Email == user.Email  || u.Login == user.Login).FirstOrDefault();
        if (isUserExist != null)
        {
            return Result.Fail<int>(ErrorHandler.RenderError(ErrorType.UserConflict));
        }
    ...
    private Result<string> Authenticate(string? login, string? password)
    {
        ...
        {
            return Result.Fail<string>(ErrorHandler.RenderError(ErrorType.BadAuthRequest));
        }
    ...
    private Result<string> Authenticate(int code)
    {
        ...
        if (user == null)
        {
            return Result.Fail<string>(ErrorHandler.RenderError(ErrorType.IncorrectConfirmCode));
        }
        user.AccessLevel = 1;
