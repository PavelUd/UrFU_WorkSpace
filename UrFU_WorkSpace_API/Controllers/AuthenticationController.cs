...
public class AuthenticationController : Controller
{
    ...
    /// </summary>
    /// <remarks>
    ///     Этот метод регистрирует нового пользователя, отправляет на указанный адрес электронной почты код подтверждения для
    ///     активации аккаунта.&#xA;
    ///     <code>Важно!</code>Код подтверждения действителен только в течение определенного времени. Если отправка кода
    ///     подтверждения не удалась, регистрация не будет завершена.&#xA;
    /// </remarks>
    ...
    public IActionResult Register([FromBody] ModifyUserDto modifyUser)
    {
        ...
        var result = _authenticationService.Register(user)
            .Then(code => _authenticationService.SendConfirmEmail(code, user.Email));
